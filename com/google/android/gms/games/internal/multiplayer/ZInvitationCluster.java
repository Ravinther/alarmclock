...
public final class ZInvitationCluster implements SafeParcelable, Invitation {
    ...
    ZInvitationCluster(int versionCode, ArrayList invitationList) {
        this.xH = versionCode;
    ...
    private void hn() {
        fb.m8420x(!this.LG.isEmpty());
        Invitation invitation = (Invitation) this.LG.get(0);
        int size = this.LG.size();
        for (int i = 1; i < size; i++) {
            fb.m8418a(invitation.getInviter().equals(((Invitation) this.LG.get(i)).getInviter()), "All the invitations must be from the same inviter");
        }
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        InvitationClusterCreator.m7713a(this, dest, flags);
    }
}
