...
class CachedVideoPlayerActivity extends VideoPlayerActivity implements Callback {
    ...
    /* renamed from: com.millennialmedia.android.CachedVideoPlayerActivity.1 */
    class C24661 implements Runnable {
        C24661() {
        }
    ...
    /* renamed from: com.millennialmedia.android.CachedVideoPlayerActivity.2 */
    class C24672 implements OnClickListener {
        final /* synthetic */ VideoImage val$button;
        final /* synthetic */ ImageButton val$newButton;

        C24672(ImageButton imageButton, VideoImage videoImage) {
    ...
    private static class CachedVideoWebViewClientListener extends MMWebViewClientListener {
        ...
        public void onPageFinished(String url) {
            Log.m9711d("@@ ON PAGE FINISHED" + url);
    ...
    private void dismissAfter(long delayMillis) {
        this.logTimeAndEventhandler.postDelayed(new C24661(), delayMillis);
    }
    ...
    private void fadeButtons() {
        if (this.videoAd != null && this.videoAd.buttons != null) {
            for (int i = 0; i < this.videoAd.buttons.size(); i++) {
                ...
                Log.m9724v("Button: %d alpha: %f", Integer.valueOf(i), Float.valueOf(videoImage.fromAlpha));
            }
        }
    }
    ...
    protected RelativeLayout initLayout() {
        ...
        if (this.videoAd == null || this.videoAd.webOverlayURL == null) {
            ...
            this.buttonsLayout.setId(LocationStatusCodes.GEOFENCE_NOT_AVAILABLE);
            List buttons = null;
            if (this.videoAd != null) {
            ...
            if (buttons != null) {
                ...
                for (int i = 0; i < buttons.size(); i++) {
                    ...
                    if (button.isLeaveBehind) {
                        ...
                        newButton.setBackgroundColor(-16777216);
                        newButtonLp = new LayoutParams(-1, -1);
                    } else {
                        newButton.setBackgroundColor(0);
                        newButtonLp = new LayoutParams(-2, -2);
                        Log.m9724v("Button: %d Anchor: %d Position: %d Anchor2: %d Position2: %d", Integer.valueOf(newButton.getId()), Integer.valueOf(button.anchor), Integer.valueOf(button.position), Integer.valueOf(button.anchor2), Integer.valueOf(button.position2));
                    ...
                    if (!TextUtils.isEmpty(button.linkUrl)) {
                        newButton.setOnClickListener(new C24672(newButton, button));
                    }
                    ...
                    if (button.inactivityTimeout > 0) {
                        Message message = Message.obtain(this.logTimeAndEventhandler, 1, button);
                        long j = button.appearanceDelay;
                        this.logTimeAndEventhandler.sendMessageDelayed(message, (button.inactivityTimeout + r0) + button.fadeDuration);
                    }
                }
    ...
    public boolean handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case C2625R.styleable.WalletFragmentStyle_buyButtonAppearance /*3*/:
                ...
                if (canFadeButtons()) {
                    fadeButton(image);
                    Log.m9724v("Beginning animation to visibility. Fade duration: %d Button: %d Time: %d", Long.valueOf(image.fadeDuration), Integer.valueOf(image.button.getId()), Long.valueOf(System.currentTimeMillis()));
                    break;
                }
                break;
        }
    ...
    protected void restartVideo() {
        if (this.videoAd != null) {
            ...
            List buttons = this.videoAd.buttons;
            this.logTimeAndEventhandler.removeMessages(1);
            this.logTimeAndEventhandler.removeMessages(2);
            this.logTimeAndEventhandler.removeMessages(3);
    ...
    private String calculateHudSecondsText() {
        if (this.currentVideoPosition != 0) {
            return String.valueOf(this.currentVideoPosition / LocationStatusCodes.GEOFENCE_NOT_AVAILABLE);
        }
        if (this.videoAd != null) {
